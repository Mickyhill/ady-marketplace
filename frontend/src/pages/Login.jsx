...
import {
  ArrowRight, BookOpen, CheckCircle2, ChevronLeft, ChevronRight, Clock3,
  Laptop, MapPin, Pause, Play, Search, ShieldCheck, ShoppingBag, Sparkles, Store, Users,
} from "lucide-react";
...
const stories = [
  {
    ...
    body: (
      <>
        <p className="text-slate-600">He needed a fairly used laptop before the semester started.</p>
        ...
        <p className="mt-3 font-semibold italic text-slate-900">"Can I trust this seller?"</p>
  ...
  },
  {
    ...
    body: (
      <>
        <p className="text-slate-600">ADY Marketplace brings AKSU buyers and sellers together in one trusted marketplace.</p>
          ...
          <div className="rounded-3xl border border-white/70 bg-white/90 p-5 shadow-2xl backdrop-blur">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold text-emerald-700">VERIFIED</div>
              </div>
            </div>
          </div>
  ...
  },
  {
    ...
    body: (
      <>
        <p className="text-slate-600">Books. Electronics. Furniture. Fashion. Services. And more.</p>
        <p className="mt-3 text-lg font-semibold text-slate-900">ADY Marketplace<br />Buy. Sell. CONNECT.</p>
      </>
    ),
    visual: (
      <div className="relative h-64 w-full">
        <div className="absolute left-1/2 top-1/2 grid w-[82%] -translate-x-1/2 -translate-y-1/2 grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="flex flex-col items-center justify-center rounded-2xl border border-white/80 bg-white/90 p-4 shadow-lg backdrop-blur transition-transform duration-300 hover:-translate-y-1">
          ...
          <div className="relative flex min-h-screen flex-col justify-between p-10 xl:p-14">
            ...
            >
              <div className="w-full">
                <div key={`visual-${activeStory}`} className="animate-[fadeUp_500ms_ease-out]">
                  {story.visual}
                </div>

                <div key={`text-${activeStory}`} className="mx-auto max-w-xl animate-[fadeUp_500ms_ease-out]">
                  <div className="mb-5 flex items-center gap-3">
                    <div className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br ${story.accent} text-white shadow-lg`}>
                      <StoryIcon className="h-5 w-5" />
                    </div>
                    <span className="text-sm font-semibold uppercase tracking-[0.18em] text-orange-600">ADY Marketplace</span>
                  </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <button type="button" onClick={previousStory} className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:text-orange-600" aria-label="Previous story">
                  <ChevronLeft className="h-5 w-5" />
                </button>
                <button type="button" onClick={nextStory} className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-orange-200 hover:text-orange-600" aria-label="Next story">
              ...
              <div className="flex items-center gap-2">
                {stories.map((_, index) => (
                  <button
                    key={index}
                    type="button"
                    onClick={(e) => { e.stopPropagation(); selectStory(index); }}
                    aria-label={`Go to story ${index + 1}`}
                    className={`h-2 rounded-full transition-all duration-300 ${activeStory === index ? "w-8 bg-orange-500" : "w-2 bg-slate-300 hover:bg-slate-400"}`}
                  />
                ))}
              </div>

              <div className="text-xs font-medium text-slate-400">{activeStory + 1} / {stories.length}</div>
            </div>
          </div>
        </section>
